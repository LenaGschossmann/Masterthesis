...
for iRoi = 1:nrois
    ...
    roi_summary(iRoi).Area_um = roidata.roi_area(tmproi);
    roi_summary(iRoi).StructNorm_Area = roidata.roi_area(tmproi)/roidata.dendritic_area;
    roi_summary(iRoi).Ctr_X_um = roidata.roi_centroids(tmproi,1);
    roi_summary(iRoi).Ctr_Y_um = roidata.roi_centroids(tmproi,2);
...
for iRoi1 = 1:nrois-1
    ...
    tmpx1 = roidata.roi_centroids(tmproi1,1);
    tmpy1 = roidata.roi_centroids(tmproi1,2);
    for iRoi2 = iRoi1+1:nrois
        % Distance
        tmproi2 = save_rois(iRoi2);
        tmpx2 = roidata.roi_centroids(tmproi2,1);
        tmpy2 = roidata.roi_centroids(tmproi2,2);
...
%         r = cov(tmpbin1, tmpbin2)./(std(tmpbin1)*std(tmpbin2));
        ...
        synchro_mtrx(iRoi1,iRoi2) = r(1,2);
...
rec_summary.Area_um_Mean = mean([roi_summary(:).EvRate]);
rec_summary.StructNorm_Area = mean([roi_summary(:).StructNorm_Area]);
rec_summary.Dend_Area_um = roidata.dendritic_area;
rec_summary.Bg_Area_um = roidata.background_area;
rec_summary.FoV_Area_um = roidata.fov_area;
rec_summary.Events_per_Dend_Area = totevents/roidata.dendritic_area;
...
saveas(f1,roi_ovw_png); close gcf;
