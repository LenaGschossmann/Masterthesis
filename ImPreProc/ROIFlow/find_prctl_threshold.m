function threshold = find_prctl_threshold(dFoFtraces, ini_thresh, mode)
% Previous input: (dFoFtraces, background, perc_thresh_range,...
%     perc_cutoff, perc_fac,connc_px_thresh, imsize, savepath, filename)
disp('Determine the appropriate threshold...');

minval = min(dFoFtraces,[],'all');
maxval = max(dFoFtraces,[],'all');
range = maxval-minval;

[n,e] = histcounts(dFoFtraces);
test = n >= max(n)/2;
hmw = e(find(test,1,'last'))-e(find(test,1,'first'));

if strncmp(mode,'C',1)
    threshold = ini_thresh;
%     if hmw/range <= 0.05, threshold = ini_thresh;
%     elseif hmw/range <= 0.1, threshold = ini_thresh-3.5;
%     else, threshold = ini_thresh-5.2;
%     end
else
    if hmw/range <= 0.05, threshold = ini_thresh;
    elseif hmw/range <= 0.1, threshold = ini_thresh-3.5;
