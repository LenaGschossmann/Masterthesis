...
if strncmp(rectype,'C',1)
    if isempty(params)
        params = get_predefined_params('connc_px_thresh', 20, 'dFoF_prctile_1', 98,'dFoF_prctile_2', 30,...
            'perc_thresh',85, 'perc_fac', 1.3,'overlap_thresh',0.5, 'fill_thresh',0.3);
    end
else
    if isempty(params)
...
%     [~, ~, dFoFtraces] = rollBase_dFoF(corr_traces, winsize,n_frames, 'grand');
    ...
    [~, ~, dFoFtraces] = rollBase_dFoF(corr_traces, winsize,n_frames, 'roll');
    ...
    if tot_num_cc ~= 0
        ...
        while combine_more
            ...
            for r = 1:tmp_num_cc-1
                ...
                %
                lindices1 = sub2ind([imsize(1) imsize(2)], tmpcc1(:,1), tmpcc1(:,2));
                ...
                for c = r+1:tmp_num_cc
                    ...
                    %
                    lindices2 = sub2ind([imsize(1) imsize(2)], tmpcc2(:,1), tmpcc2(:,2));
...
%             overlap_comb = logical(overlap_mtrx > params.overlap_thresh);
            overlap_comb = logical(overlap_mtrx > params.overlap_thresh & corr_mtrx >= 0.7);
