function [roivals, roidFoF,roiFoF, yrange, avvals, alldFoF, allFoF] = calc_roi_av_trace(roiidx, vals, pr)

% Declare globally shared variables
global roiINFO FTIME dFWIN
...
collapsex = sum(roimask,2);
ymin = find(collapsex,1,'first'); ymax = find(collapsex,1,'last');
% dFoF
disp('Calculate dF/F trace...');
lin_avvals = avvals';
[allFoF, alldFoF] = rollBase_dFoF(lin_avvals,deltawinsz,size(lin_avvals,2), 'roll');
allFoF = allFoF'; alldFoF = alldFoF';
roivals = avvals(pr(1)+ymin-1:pr(1)+ymax-1,1);
roiFoF = allFoF(pr(1)+ymin-1:pr(1)+ymax-1,1);
roidFoF = alldFoF(pr(1)+ymin-1:pr(1)+ymax-1,1);
