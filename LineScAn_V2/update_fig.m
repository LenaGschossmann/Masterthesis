...
global WHFIG FONTSIZE SCMAP WINSZ PLOTRANGE CLIMRAW CLIMUI...
...
if ROICNTER > 0
    scrawroi = mark_rois(scraw,'all');
    scavroi = mark_rois(scav,'all');
    dFoFroi = mark_rois(dFoF,'all');
else
    scrawroi=scraw;
    scavroi=scav;
    dFoFroi=dFoF;
...
title(spname);
colorbar;
set(im1, 'ButtonDownFcn', {@cb_subplot, scraw, CLIMRAW});
% Averaged image
...
title(spname);
colorbar;
set(im2, 'ButtonDownFcn', {@cb_subplot, scav, CLIMRAW});
% DeltaFovF
...
title(spname);
colorbar;
set(im3, 'ButtonDownFcn', {@cb_subplot,dFoF, CLIMUI});

%% Display Info
currinfo = {'Information:', '', sprintf('Plotrange: %i - %i', PLOTRANGE),...
    '', sprintf('Image size (post binning): W: %i | H: %i',IMHW(2), IMHW(1)),...
    '', sprintf('Line averaging window size: %i',WINSZ),...
    '', sprintf('Colormap: %s', SCMAP),...
    '', sprintf('Color scale limits: %i - %i', CLIMUI),...
...
%% Local callbacks
    function cb_subplot(~,~,tmpmtrx, csclims)
        create_rois(tmpmtrx,csclims);
    end
