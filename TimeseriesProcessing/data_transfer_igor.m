...
rgbmap = [204, 0, 4;...
    77, 0, 153;...
    0, 153, 77;...
    0, 102, 153]; % CTRL; TTX; A184V; A184S
custommap = rgbmap./256;

% Load
data = readtable('C:\Users\lena_\Projects\iGluSnFR\Analysis\Analysis_Master_V4\Summary_Confocal_V4.xlsx', 'Sheet',5);
% data = readtable('C:\Users\lena_\Projects\iGluSnFR\Analysis\Analysis_Master_V4\Summary_2PM_V4.xlsx', 'Sheet',2);
...
sensor ='A184V';
cond = 'CTRL';
% val1 = data.Area_um_Mean(strcmp(data.Sensor, sensor) & strcmp(data.CTRL_TTX_STIM, cond));
% val1 = data.ToT_Num_Events(strcmp(data.Sensor, sensor) & strcmp(data.CTRL_TTX_STIM, cond));
val1 = data.Events_per_Dend_Area(strcmp(data.Sensor, sensor) & strcmp(data.CTRL_TTX_STIM, cond));
val2 = data.rel_ML_pos(strcmp(data.Sensor, sensor) & strcmp(data.CTRL_TTX_STIM, cond));
...
% rel_ML_pos
sumvar = 'Peak_dFoF_Mean'; sumpter = find(strcmp(data.Properties.VariableNames, sumvar)==1);
% Amp_FoF_Mean Num_Events Area_um EvRate_Hz Peak_dFoF_Mean
splitsum_tbl = table();
splitsum_tbl.A184V_CTRL = NaN(numel(borders)-1,1);
splitsum_tbl.A184V_TTX = NaN(numel(borders)-1,1);
splitsum_tbl.A18S_CTRL = NaN(numel(borders)-1,1);
splitsum_tbl.A184S_TTX = NaN(numel(borders)-1,1);
...
%     tmpdata = data(strcmp(data.Sensor, 'A184V') & data.Area >= 50,:);
    ...
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184V') & strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'),sumpter));
    tmp(isnan(tmp)) = [];
    splitsum_tbl.A184V_CTRL(i) = mean(tmp);
    splitci_tbl.A184V_CTRL(i) = 1.96* std(tmp)/sqrt(numel(tmp));
    splitci1_tbl.A184V_CTRL(i) = mean(tmp) - 1.96* std(tmp)/sqrt(numel(tmp));
    splitci2_tbl.A184V_CTRL(i) = mean(tmp) + 1.96* std(tmp)/sqrt(numel(tmp));
%     splitn_tbl.A184V_CTRL(i) = numel(tmp);
    splitn_tbl.A184V_CTRL(i) = numel(unique(tmpdata.Recording(strcmp(tmpdata.Sensor, 'A184V')& strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'))));
    anova_val = [anova_val; tmp];
    anova_grp1 = [anova_grp1; repelem(1, size(tmp,1))'];
    anova_grp2 = [anova_grp2; repelem(i, size(tmp,1))'];
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184V') & strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'),sumpter));
    tmp(isnan(tmp)) = [];
    splitsum_tbl.A184V_TTX(i) = mean(tmp);
    splitci_tbl.A184V_TTX(i) = 1.96* std(tmp)/sqrt(numel(tmp));
    splitci1_tbl.A184V_TTX(i) = mean(tmp) - 1.96* std(tmp)/sqrt(numel(tmp));
    splitci2_tbl.A184V_TTX(i) = mean(tmp) + 1.96* std(tmp)/sqrt(numel(tmp));
%     splitn_tbl.A184V_TTX(i) = numel(tmp);
    splitn_tbl.A184V_TTX(i)= numel(unique(tmpdata.Recording(strcmp(tmpdata.Sensor, 'A184V')& strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'))));
    anova_val = [anova_val; tmp];
    anova_grp1 = [anova_grp1; repelem(2, size(tmp,1))'];
    anova_grp2 = [anova_grp2; repelem(i, size(tmp,1))'];
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184S') & strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'),sumpter));
    tmp(isnan(tmp)) = [];
    splitsum_tbl.A18S_CTRL(i) = mean(tmp);
    splitci_tbl.A18S_CTRL(i) = 1.96* std(tmp)/sqrt(numel(tmp)); 
    splitci1_tbl.A18S_CTRL(i) = mean(tmp) - 1.96* std(tmp)/sqrt(numel(tmp)); 
    splitci2_tbl.A18S_CTRL(i) = mean(tmp) + 1.96* std(tmp)/sqrt(numel(tmp));
%     splitn_tbl.A18S_CTRL(i) = numel(tmp);
    splitn_tbl.A18S_CTRL(i)= numel(unique(tmpdata.Recording(strcmp(tmpdata.Sensor, 'A184S')& strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'))));
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184S') & strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'),sumpter));
    tmp(isnan(tmp)) = [];
    splitsum_tbl.A184S_TTX(i) = mean(tmp);
    splitci_tbl.A184S_TTX(i) = 1.96* std(tmp)/sqrt(numel(tmp)); 
    splitci1_tbl.A184S_TTX(i) = mean(tmp) - 1.96* std(tmp)/sqrt(numel(tmp)); 
    splitci2_tbl.A184S_TTX(i) = mean(tmp) + 1.96* std(tmp)/sqrt(numel(tmp));
%     splitn_tbl.A184S_TTX(i) = numel(tmp);
    splitn_tbl.A184S_TTX(i)= numel(unique(tmpdata.Recording(strcmp(tmpdata.Sensor, 'A184S')& strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'))));
...
g(1,1) = gramm('x', xticks, 'y', splitsum_tbl.A184V_CTRL,...
    'ymin', splitci1_tbl.A184V_CTRL,'ymax', splitci2_tbl.A184V_CTRL);
...
g(2,1) = gramm('x', xticks, 'y', splitsum_tbl.A184V_TTX,...
    'ymin', splitci1_tbl.A184V_TTX,'ymax', splitci2_tbl.A184V_TTX);
...
g.axe_property('YLIM', [0 max([splitci2_tbl.A184V_CTRL; splitci2_tbl.A184V_TTX])])
g.set_title(strcat(sumvar, '(Mean & CI)')); 
g.draw();

%% Find means in certain ranges
tmp = table2array(data(strcmp(data.Sensor, 'A184V') & strcmp(data.CTRL_TTX_STIM, 'TTX') & data.Area >= 50,:));
%& data.rel_ML_pos < 40   & strcmp(data.CTRL_TTX_STIM, 'TTX') 
mean(tmp, 'omitnan')
numel(tmp)

%% Graph n
xticks = borders(1:end-1) + diff(borders)/2;
gn(1,1) = gramm('x', xticks, 'y', splitn_tbl.A184V_CTRL);
...
gn(2,1) = gramm('x', xticks, 'y', splitn_tbl.A184V_TTX);
...
gn.axe_property('YLIM', [0 max([splitn_tbl.A184V_CTRL; splitn_tbl.A184V_TTX])])
gn.set_title('Units'); 
gn.draw();
...
sum_tbl.cond(1:4) = {'CTRL','TTX','CTRL','TTX'};
% sum_tbl.sensor(1:2) = {'A184V','A184S'};
% sum_tbl.cond(1:2) = {'CTRL','CTRL'};
anova_val = [];
anova_grp1 = [];
anova_grp2 = [];

for iC = 1:numel(sum_tbl.cond)
    tmpdata = data(strcmp(data.Sensor, sum_tbl.sensor(iC)) & strcmp(data.CTRL_TTX_STIM, sum_tbl.cond(iC)),:);
    sum_tbl.FoF_peak_mean(iC) = mean(tmpdata.Peak_dFoF_Mean);
    sum_tbl.FoF_peak_ci(iC) = 1.96*std(tmpdata.Peak_dFoF_Mean)/sqrt(numel(tmpdata.Peak_dFoF_Mean));
    sum_tbl.FoF_peak_sem(iC) = std(tmpdata.Peak_dFoF_Mean)/sqrt(numel(tmpdata.Peak_dFoF_Mean));
    sum_tbl.roi_area_mean(iC) = mean(tmpdata.Area_um_Mean);
    sum_tbl.roi_area_ci(iC) = 1.96*std(tmpdata.Area_um_Mean)/sqrt(numel(tmpdata.Area_um_Mean));
    sum_tbl.roi_area_sem(iC) = std(tmpdata.Area_um_Mean)/sqrt(numel(tmpdata.Area_um_Mean));
    sum_tbl.num_rois_mean(iC) = mean(tmpdata.Num_ROIs);
    sum_tbl.num_rois_ci(iC) = 1.96*std(tmpdata.Num_ROIs)/sqrt(numel(tmpdata.Num_ROIs));
    sum_tbl.num_rois_sem(iC) = std(tmpdata.Num_ROIs)/sqrt(numel(tmpdata.Num_ROIs));
    sum_tbl.num_ev_mean(iC) = mean(tmpdata.ToT_Num_Events);
    sum_tbl.num_ev_ci(iC) = 1.96*std(tmpdata.ToT_Num_Events)/sqrt(numel(tmpdata.ToT_Num_Events));
    sum_tbl.num_ev_sem(iC) = std(tmpdata.ToT_Num_Events)/sqrt(numel(tmpdata.ToT_Num_Events));
    ...
    sum_tbl.num_evproi_sem(iC) = std(tmpdata.Num_Events_Mean)/sqrt(numel(tmpdata.Num_Events_Mean));
    ...
    sum_tbl.evperdend_sem(iC) = std(tmpdata.Events_per_Dend_Area)/sqrt(numel(tmpdata.Events_per_Dend_Area));
    ...
    sum_tbl.dend_area_sem(iC) = std(tmpdata.Dend_Area_um)/sqrt(numel(tmpdata.Dend_Area_um));
    sum_tbl.norm_roi_area_mean(iC) = mean(tmpdata.Area_ROI_PercOf_Struct);
    sum_tbl.norm_roi_area_ci_hspan(iC) = 1.96*std(tmpdata.Area_ROI_PercOf_Struct)/sqrt(numel(tmpdata.Area_ROI_PercOf_Struct));
    sum_tbl.norm_roi_area_sem(iC) = std(tmpdata.Area_ROI_PercOf_Struct)/sqrt(numel(tmpdata.Area_ROI_PercOf_Struct));
%     sum_tbl.ev_per_norm_roi_area_mean(iC) = mean(tmpdata.ToT_Num_Events*100/tmpdata.Area_ROI_PercOf_Struct);
%     sum_tbl.norm_roi_area_ci_hspan(iC) = 1.96*std(tmpdata.Area_ROI_PercOf_Struct)/sqrt(numel(tmpdata.Area_ROI_PercOf_Struct));
...
splitn_noact_tbl.A184V_CTRL = NaN(numel(borders)-1,1);
splitn_noact_tbl.A184V_TTX = NaN(numel(borders)-1,1);
splitn_noact_tbl.A18S_CTRL = NaN(numel(borders)-1,1);
splitn_noact_tbl.A184S_TTX = NaN(numel(borders)-1,1);
...
for i = 1:numel(borders)-1
    ...
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184V') & strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'),5));
    splitn_noact_tbl.A184V_CTRL(i) = numel(tmp);
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184V') & strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'),5));
    splitn_noact_tbl.A184V_TTX(i) = numel(tmp);
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184S') & strcmp(tmpdata.CTRL_TTX_STIM, 'CTRL'),5));
    splitn_noact_tbl.A18S_CTRL(i) = numel(tmp);
    
    tmp = table2array(tmpdata(strcmp(tmpdata.Sensor, 'A184S') & strcmp(tmpdata.CTRL_TTX_STIM, 'TTX'),5));
    splitn_noact_tbl.A184S_TTX(i) = numel(tmp);
end

% Graph n
xticks = borders(1:end-1) + diff(borders)/2;
gn(1,1) = gramm('x', xticks, 'y', splitn_noact_tbl.A184V_CTRL);
gn(1,1).geom_bar('dodge',0.1,'width',0.8);
gn(1,1).set_color_options('map', custommap(1,:)); 

gn(2,1) = gramm('x', xticks, 'y', splitn_noact_tbl.A184V_TTX);
gn(2,1).geom_bar('dodge',0.1,'width',0.8);
gn(2,1).set_color_options('map', custommap(2,:));

gn.set_names('x', 'Rel. ML position', 'y', '# units');
gn.axe_property('XLIM', [0 110], 'XTick', borders(2:end));
gn.axe_property('YLIM', [0 max([splitn_noact_tbl.A184V_CTRL; splitn_noact_tbl.A184V_TTX])])
...
data2pm = readtable('C:\Users\lena_\Projects\iGluSnFR\Analysis\Analysis_Master_V4\Summary_2PM_V4.xlsx', 'Sheet',4);
...
for iC = 1:2
    ...
    sum_tbl.dFoF_amp_mean(iC) = mean(tmpdata.Amp_FoF_Mean);
    sum_tbl.dFoF_amp_ci_hspan(iC) = 1.96*std(tmpdata.Amp_FoF_Mean)/sqrt(numel(tmpdata.Amp_FoF_Mean));
    sum_tbl.roi_area_mean(iC) = mean(tmpdata.Area_um);
    sum_tbl.roi_area_ci_hspan(iC) = 1.96*std(tmpdata.Area_um)/sqrt(numel(tmpdata.Area_um));
    sum_tbl.num_rois_mean(iC) = mean(tmpdata.Num_ROIs);
...
tmpdata(isnan(tmpdata.PearsonR),:)=[];
...
%     mask = tmpdata.PearsonR >= distbins(iD) & tmpdata.PearsonR < distbins(iD+1);
    ...
    result_mtrx.EvRate_av(iD) = mean(tmpdata.AverageEventrate(mask));
%     result_mtrx.EvRate_CI(iD) = 1.96*std(tmpdata.MeanEvRate(mask))/sqrt(numel(tmpdata.MeanEvRate(mask)));
    ...
    anova_val = [anova_val; tmpdata.AverageEventrate(mask)];
